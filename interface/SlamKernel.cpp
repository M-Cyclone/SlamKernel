...
SlamKernel::SlamKernel(int32_t     img_width,
                       int32_t     img_height,
...
// unique_ptr needs to know how to delete the ptr, so the dtor should be impl
// with the definition of the ptr class.
...
TrackingResult SlamKernel::handleData(const std::vector<Image>&    images,
                                      const std::vector<ImuPoint>& imus)
{
    ...
    memcpy(
        cv_image.data, image.data.data(), sizeof(uint8_t) * image.data.size());
    ...
    {
        orb_imus.emplace_back(-ax,
                              ay,
                              az,
                              -wx,
                              wy,
                              wz,
                              (double)(ts - m_begin_time_stamp)
                                  * k_nano_sec_to_sec_radio);
    }

    // Get time stamp from android time stamp.
    double image_time_stamp = (double)(image.time_stamp - m_begin_time_stamp)
                              * k_nano_sec_to_sec_radio;
    // Tracking.
    Sophus::SE3f pose =
        m_orb_slam->TrackMonocular(cv_image, image_time_stamp, orb_imus);
    ...
    {
        {
            std::vector<ORB_SLAM3::KeyFrame*> key_frames =
                active_map->GetAllKeyFrames();

            static auto key_frame_cmp =
                [](const ORB_SLAM3::KeyFrame* kf1,
                   const ORB_SLAM3::KeyFrame* kf2) -> bool {
                if (!kf1 || !kf2)
                    return true;

                return kf1->mnFrameId < kf2->mnFrameId;
            };
            std::set<ORB_SLAM3::KeyFrame*, decltype(key_frame_cmp)> kf_set(
                key_frame_cmp);
        ...
        {
            std::vector<ORB_SLAM3::MapPoint*> local_mps =
                active_map->GetReferenceMapPoints();
            std::unordered_set<ORB_SLAM3::MapPoint*> local_mp_ust(
                local_mps.begin(), local_mps.end());
            ...
            for (ORB_SLAM3::MapPoint* mp : local_mps)
            {
                ...
                res.map_points.push_back(
                    {position.x(), position.y(), position.z()});
            }

            std::vector<ORB_SLAM3::MapPoint*> all_mps =
                active_map->GetAllMapPoints();
            for (ORB_SLAM3::MapPoint* mp : all_mps)
            {
                if (!mp || mp->isBad()
                    || local_mp_ust.find(mp) != local_mp_ust.end())
                    continue;
    ...
    // Processing time cost.
    {
        ...
        res.processing_delta_time =
            std::chrono::duration<float>(m_last_time - curr_time).count();
    }
