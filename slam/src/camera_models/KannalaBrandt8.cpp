...
cv::Point2f KannalaBrandt8::project(const cv::Point3f& p3D)
{
    ...
    const float r      = theta + mvParameters[4] * theta3 +
                    mvParameters[5] * theta5 + mvParameters[6] * theta7 +
...
Eigen::Vector2d KannalaBrandt8::project(const Eigen::Vector3d& v3D)
{
    ...
    const double r      = theta + mvParameters[4] * theta3 +
                     mvParameters[5] * theta5 + mvParameters[6] * theta7 +
                     mvParameters[7] * theta9;
    ...
    const Eigen::Vector3d& v3D)
{
    ...
    double fd = 1 + 3 * mvParameters[4] * theta2 +
                5 * mvParameters[5] * theta4 + 7 * mvParameters[6] * theta6 +
