...
std::vector<KeyFrame*> KeyFrame::GetCovisiblesByWeight(const int& w)
{
    ...
    auto it = upper_bound(mvOrderedWeights.begin(),
...
void KeyFrame::UpdateConnections(bool upParent)
{
    ...
    for (auto mit = KFcounter.begin(), mend = KFcounter.end(); mit != mend;
         mit++)
    {
        if (!upParent)
...
void KeyFrame::SetBadFlag()
{
...

    {
        ...
        {
            for (auto sit = mspChildrens.begin(); sit != mspChildrens.end();
                 sit++)
            {
                                                ...
                                                const bool   bRight) const
{
    ...
    const int nMinCellX = std::max(
        0, (int)std::floor((x - mnMinX - factorX) * mfGridElementWidthInv));
    ...
    const int nMinCellY = std::max(
        0, (int)std::floor((y - mnMinY - factorY) * mfGridElementHeightInv));
...
void KeyFrame::PreSave(std::set<KeyFrame*>&        spKF,
                        ...
                        std::map<unsigned int, GeometricCamera*>& mpCamId)
{
    ...
    mConnectedKeyFrameWeights.clear();
    for (auto it  = mBackupConnectedKeyFrameIdWeights.cbegin(),
              end = mBackupConnectedKeyFrameIdWeights.cend();
         it != end;
         ++it)
    {
    ...
    // KeyFrame childrens
    mspChildrens.clear();
    for (auto it  = mvBackupChildrensId.cbegin(),
              end = mvBackupChildrensId.cend();
         it != end;
         ++it)
    {
    ...
    // Loop edge KeyFrame
    mspLoopEdges.clear();
    for (auto it  = mvBackupLoopEdgesId.cbegin(),
              end = mvBackupLoopEdgesId.cend();
         it != end;
         ++it)
    {
    ...
    // Merge edge KeyFrame
    mspMergeEdges.clear();
    for (auto it  = mvBackupMergeEdgesId.cbegin(),
              end = mvBackupMergeEdgesId.cend();
         it != end;
         ++it)
    {
