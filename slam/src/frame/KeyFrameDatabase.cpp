...
void KeyFrameDatabase::add(KeyFrame* pKF)
{
    ...
    for (auto vit = pKF->mBowVec.cbegin(), vend = pKF->mBowVec.cend();
         vit != vend;
         vit++)
...
void KeyFrameDatabase::clearMap(Map* pMap)
{
    ...
    // Erase elements in the Inverse File for the entry
    for (auto vit = mvInvertedFile.begin(), vend = mvInvertedFile.end();
         vit != vend;
         vit++)
    {
        // List of keyframes that share the word
                                        ...
                                        vector<KeyFrame*>& vpMergeCand)
{
    ...
    if (!lKFsSharingWordsLoop.empty())
    {
        ...
        if (!lScoreAndMatch.empty())
        {
                 ...
                 it++)
            {
                ...
                float     accScore  = it->first;
                KeyFrame* pBestKF   = pKFi;
                for (auto vit = vpNeighs.begin(), vend = vpNeighs.end();
                     vit != vend;
                     vit++)
                {
                    KeyFrame* pKF2 = *vit;
    ...
    if (!lKFsSharingWordsMerge.empty())
    {
        ...
        // Only compare against those keyframes that share enough words
        int maxCommonWords = 0;
        for (auto lit  = lKFsSharingWordsMerge.begin(),
                                                                   ...
                                                                   Map*   pMap)
{
    ...
    // Search all keyframes that share a word with current frame
    {
        ...
        for (auto vit = F->mBowVec.cbegin(), vend = F->mBowVec.cend();
             vit != vend;
             vit++)
        {
            list<KeyFrame*>& lKFs = mvInvertedFile[vit->first];
