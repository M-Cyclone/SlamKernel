...
void Map::AddKeyFrame(KeyFrame* pKF)
{
    ...
    {
        std::cout << "First KF:" << pKF->mnId << "; Map init KF:" << mnInitKFid
                  << std::endl;
        mnInitKFid  = pKF->mnId;
        mpKFinitial = pKF;
        mpKFlowerID = pKF;
...
void Map::EraseKeyFrame(KeyFrame* pKF)
{
    ...
    if (mspKeyFrames.size() > 0)
    {
        if (pKF->mnId == mpKFlowerID->mnId)
        {
            std::vector<KeyFrame*> vpKFs(mspKeyFrames.begin(),
                                         mspKeyFrames.end());
...
void Map::clear()
{
    ...
    for (auto sit = mspKeyFrames.begin(), send = mspKeyFrames.end();
         sit != send;
         sit++)
    {
        KeyFrame* pKF = *sit;
...
void Map::PreSave(std::set<GeometricCamera*>& spCams)
{
    ...
    for (MapPoint* pMPi : mspMapPoints)
    {
        ...
        std::map<KeyFrame*, std::tuple<int, int>> mpObs =
