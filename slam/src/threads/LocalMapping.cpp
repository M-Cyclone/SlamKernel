        ...
        if (CheckNewKeyFrames() && !mbBadImu)
        {
            ...
            if (!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                if ((mTinit < 50.0f) && mbInertial)
                {
                    if (mpCurrentKeyFrame->GetMap()->isImuInitialized()
                        && mpTracker->mState
                               == Tracking::OK)  // Enter here everytime
...
void LocalMapping::CreateNewMapPoints()
{
    ...
    if (mbMonocular)
        nn = 30;
    std::vector<KeyFrame*> vpNeighKFs =
    ...
    for (size_t i = 0; i < vpNeighKFs.size(); i++)
    {
        ...
        for (int ikp = 0; ikp < nmatches; ikp++)
        {
            ...
            {
                goodProj = GeometricTools::Triangulate(
                    xn1, xn2, eigTcw1, eigTcw2, x3D);
                if (!goodProj)
                    continue;
            }
...
void LocalMapping::SearchInNeighbors()
{
    ...
    for (auto vit = vpNeighKFs.cbegin(), vend = vpNeighKFs.cend(); vit != vend;
         vit++)
    {
        KeyFrame* pKFi = *vit;
    ...
    for (int i = 0, imax = vpTargetKFs.size(); i < imax; i++)
    {
        ...
        for (auto vit2  = vpSecondNeighKFs.cbegin(),
                  vend2 = vpSecondNeighKFs.cend();
             vit2 != vend2;
             vit2++)
        {
            KeyFrame* pKFi2 = *vit2;
...
void LocalMapping::Release()
{
    ...
    mbStopped       = false;
    mbStopRequested = false;
    for (auto lit = mlNewKeyFrames.begin(), lend = mlNewKeyFrames.end();
         lit != lend;
         lit++)
        delete *lit;
            ...
            if (pMP)
            {
                if (!pMP->isBad())
                {
                    ...
                    if (pMP->Observations() > thObs)
                    {
                        ...
                        const std::map<KeyFrame*, std::tuple<int, int>>
                            observations = pMP->GetObservations();
                        int nObs         = 0;
                        for (auto mit  = observations.cbegin(),
                                  mend = observations.cend();
                             mit != mend;
                             mit++)
                        {
...
void LocalMapping::ResetIfRequested()
{
    bool executed_reset = false;
    {
        ...
        {
            executed_reset = true;
            std::cout << "LM: Reseting current map in Local Mapping..."
                      << std::endl;
            mlNewKeyFrames.clear();
            mlpRecentAddedMapPoints.clear();
