            ...
            if (bFindedRegion)
            {
                if (mbMergeDetected)
                {
                    ...
                    else
                    {
                        ...
                        g2o::Sim3 gSmw2(
                            mTmw.unit_quaternion(), mTmw.translation(), 1.0);
                        Sophus::SE3d mTcw =
                            mpCurrentKF->GetPose().cast<double>();
                        g2o::Sim3 gScw1(
                            mTcw.unit_quaternion(), mTcw.translation(), 1.0);
                ...
                if (mbLoopDetected)
                {
                    ...
                    if (mpCurrentKF->GetMap()->IsInertial())
                    {
                        ...
                        g2o::Sim3 g2oTwc(
                            Twc.unit_quaternion(), Twc.translation(), 1.0);
                    ...
                    if (bGoodLoop)
                    {
                        ...
                        double timeLoopTotal =
                            std::chrono::duration_cast<
                                std::chrono::duration<double, std::milli>>(
                                time_std::endloop - time_StartLoop)
                                .count();
...
bool LoopClosing::NewDetectCommonRegions()
{
    ...
    {
        // std::cout << "LoopClousure: Stereo KF inserted without check: " <<
        // mpCurrentKF->mnId << std::endl;
        mpKeyFrameDB->add(mpCurrentKF);
        mpCurrentKF->SetErase();
        return false;
    }
    ...
    {
        // std::cout << "LoopClousure: Stereo KF inserted without check, map is
        // small: " << mpCurrentKF->mnId << std::endl;
        mpKeyFrameDB->add(mpCurrentKF);
        mpCurrentKF->SetErase();
        return false;
    }

    // std::cout << "LoopClousure: Checking KF: " << mpCurrentKF->mnId <<
    // std::endl;
    ...
    for (KeyFrame* pKFi : vpBowCand)
    {
        ...
        if (numBoWMatches >= nBoWMatches)  // TODO pick a good threshold
        {
            ...
            if (bConverge)
            {
                ...
                std::set<KeyFrame*> spCheckKFs(vpCovKFi.begin(),
                                               vpCovKFi.end());
                ...
                int numProjMatches = matcher.SearchByProjection(mpCurrentKF,
                                                                ...
                                                                8,
                                                                1.5);
                // std::cout <<"BoW: " << numProjMatches << " matches between "
                // << vpMapPoints.size() << " points with coarse Sim3" <<
                // std::endl;
...
bool LoopClosing::DetectCommonRegionsFromLastKF(
    ...
    std::vector<MapPoint*>& vpMatchedMPs)
{
    std::set<MapPoint*> spAlreadyMatchedMPs(vpMatchedMPs.begin(),
                                            vpMatchedMPs.end());
    nNumProjMatches = FindMatchesByProjection(
        pCurrentKF, pMatchedKF, gScw, spAlreadyMatchedMPs, vpMPs, vpMatchedMPs);
    ...
    std::vector<MapPoint*>& vpMatchedMapPoints)
{
    ...
    std::set<KeyFrame*> spCurrentCovisbles =
...
void LoopClosing::CorrectLoop()
{
    ...
    {
        // std::cout << "Loop -> Scale correction: " << mg2oLoopScw.scale() <<
        // std::endl;
...
void LoopClosing::MergeLocal()
{
    ...
    // Verbose::VERBOSITY_DEBUG); std::cout << "Request Stop Local Mapping" <<
    // std::endl;
    mpLocalMapper->RequestStop();
    // Wait until Local Mapping has effectively stopped
    while (!mpLocalMapper->isStopped())
                ...
                if (pCurrentMap->isImuInitialized())
                {
                    Eigen::Quaternionf Rcor =
                         ...
                         * vNonCorrectedSim3[pKFi].rotation())
                            .cast<float>();
                    pKFi->SetVelocity(
                        Rcor * pKFi->GetVelocity());  // TODO: should add here
                                                      // scale s
                }
            }
    ...
    if (bRelaunchBA
        && (!pCurrentMap->isImuInitialized()
            || (pCurrentMap->KeyFramesInMap() < 200
                && mpAtlas->CountMaps() == 1)))
    {
...
void LoopClosing::MergeLocal2()
{
...

    {
        ...
        std::unique_lock<std::mutex> lock(
            mpAtlas->GetCurrentMap()->mMutexMapUpdate);

        // std::cout << "KFs before empty: " <<
        // mpAtlas->GetCurrentMap()->KeyFramesInMap() << std::endl;
        mpLocalMapper->EmptyQueue();
        // std::cout << "KFs after empty: " <<
        // mpAtlas->GetCurrentMap()->KeyFramesInMap() << std::endl;
        ...
        // std::cout << "curr tracking KF id: " <<
        // mpTracker->GetLastKeyFrame()->mnId
        // << std::endl;
    ...
    // std::cout << "MergeMap init ID: " << pMergeMap->GetInitKFid() << "
    // CurrMap init ID: " << pCurrentMap->GetInitKFid() << std::endl;
    ...
    // std::cout << "end updating current map" << std::endl;
    ...
    std::vector<KeyFrame*> aux =
        mpMergeMatchedKF->GetVectorCovisibleKeyFrames();
    ...
    /*std::cout << "vpCurrentConnectedKFs.size() " <<
    vpCurrentConnectedKFs.size() << std::endl; std::cout <<
    "mvpMergeConnectedKFs.size() " << mvpMergeConnectedKFs.size()
    << std::endl; std::cout << "spMapPointMerge.size() " <<
    spMapPointMerge.size() << std::endl;*/
...
void LoopClosing::CheckObservations(std::set<KeyFrame*>& spKFsMap1,
                                    std::set<KeyFrame*>& spKFsMap2)
{
    ...
    for (KeyFrame* pKFi1 : spKFsMap1)
    {
        ...
        for (MapPoint* pMPij : spMPs)
        {
            ...
            std::map<KeyFrame*, std::tuple<int, int>> mMPijObs =
        ...
        {
            std::cout << "CHECK-OBS: KF " << pKFi1->mnId
        ...
        {
            std::cout << "CHECK-OBS: KF " << pKFi1->mnId
                      << " has matched MP with " << mMatchedMP.size()
                      << " KF from the other map" << std::endl;
            for (std::pair<KeyFrame*, int> matchedKF : mMatchedMP)
            {
                std::cout << "   -KF: " << matchedKF.first->mnId
                          << ", Number of matches: " << matchedKF.second
                          << std::endl;
            }
        }
    }
                                ...
                                std::vector<MapPoint*>& vpMapPoints)
{
    ...
    // std::cout << "[FUSE]: Initially there are " << vpMapPoints.size() << "
    // MPs" << std::endl; std::cout << "FUSE: Intially there are " <<
    // CorrectedPosesMap.size() << " KFs" << std::endl;
    ...
    // std::cout << "[FUSE]: " << total_replaces << " MPs had been fused" <<
    // std::endl;
}


void LoopClosing::SearchAndFuse(const std::vector<KeyFrame*>& vConectedKFs,
                                std::vector<MapPoint*>&       vpMapPoints)
{
    ORBmatcher matcher(0.8);
    ...
    // std::cout << "FUSE-POSE: Initially there are " << vpMapPoints.size() << "
    // MPs"
    // << std::endl; std::cout << "FUSE-POSE: Intially there are " <<
    // vConectedKFs.size()
    // << " KFs" << std::endl;
         ...
         mit++)
    {
        ...
        /*std::cout << "FUSE-POSE: KF " << pKF->mnId << " ->" << num_replaces <<
        " MPs fused" << std::endl; total_replaces += num_replaces;*/
    }
    // std::cout << "FUSE-POSE: " << total_replaces << " MPs had been fused" <<
    // std::endl;
}
...
void LoopClosing::ResetIfRequested()
{
    ...
    else if (mbResetActiveMapRequested)
    {
        ...
        for (auto it = mlpLoopKeyFrameQueue.cbegin();
             it != mlpLoopKeyFrameQueue.cend();)
        {
            KeyFrame* pKFi = *it;
    ...
    // tree
    {
        ...
        if (!mbStopGBA)
        {
            ...
            while (!lpKFtoCheck.empty())
            {
                ...
                // std::cout << "---Updating KF " << pKF->mnId << " with " <<
                // sChilds.size() << " childs" << std::endl; std::cout << " KF
                // mnBAGlobalForKF: " << pKF->mnBAGlobalForKF << std::endl;
                     ...
                     sit++)
                {
                    ...
                    {
                        // std::cout << "++++New child with flag " <<
                        // pChild->mnBAGlobalForKF << "; LoopKF: " << nLoopKF <<
                        // std::endl; std::cout << " child id: " << pChild->mnId
                        // << std::endl;
                        Sophus::SE3f Tchildc = pChild->GetPose() * Twc;
                        // std::cout << "Child pose: " << Tchildc << std::endl;
                        // std::cout << "pKF->mTcwGBA: " << pKF->mTcwGBA <<
                        // std::endl;
                        pChild->mTcwGBA =
                        ...
                        // std::cout << "Child bias: " << pChild->GetImuBias()
                        // << std::endl;
                ...
                // std::cout << "pKF->mTcwBefGBA: " << pKF->mTcwBefGBA <<
                // std::endl;
                pKF->SetPose(pKF->mTcwGBA);
                /*cv::Mat Tco_cn = pKF->mTcwBefGBA * pKF->mTcwGBA.inv();
                cv::Vec3d trasl = Tco_cn.rowRange(0,3).col(3);
                double dist = cv::norm(trasl);
                std::cout << "GBA: KF " << pKF->mnId << " had been moved " <<
                dist << " meters" << std::endl; double desvX = 0; double desvY =
                0; double desvZ = 0; if(pKF->mbHasHessian)
                {
                ...
                pKF->GetConnectedKeyFrames().size() << " connected KFs" <<
                std::endl; std::cout << "--It has " <<
                pKF->GetCovisiblesByWeight(80).size()
                << " connected KF with 80 common matches or more" << std::endl;
                std::cout
                << "--It has " << pKF->GetCovisiblesByWeight(50).size() << "
                connected KF with 50 common matches or more" << std::endl;
                std::cout
                <<
                "--It has " << pKF->GetCovisiblesByWeight(20).size() << "
                connected KF with 20 common matches or more" << std::endl;

                    std::cout << "--STD in meters(x, y, z): " << desvX << ", "
                << desvY << ", " << desvZ << std::endl;
                ...
                CV_LOAD_IMAGE_UNCHANGED);
                    ...
                    std::vector<MapPoint*> vpMapPointsKF =
                pKF->GetMapPointMatches(); int num_MPs = 0; for(int i=0;
                i<vpMapPointsKF.size(); ++i)
                    {
                ...
                CV_FONT_HERSHEY_DUPLEX, 1, cv::Scalar(255, 0, 0));
                    ...
                    std::cout << "--It has " << num_MPs << " MPs matched in the
                map"
                << std::endl;
                ...
                if (pKF->bImu)
                {
                    // std::cout << "-------Update inertial values" <<
                    // std::endl;
            ...
            double timeUpdateMap =
                std::chrono::duration_cast<
                    std::chrono::duration<double, std::milli>>(time_EndUpdateMap
                                                               - time_EndGBA)
                    .count();
