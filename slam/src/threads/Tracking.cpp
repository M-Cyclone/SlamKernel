    ...
    , mpLastKeyFrame(static_cast<KeyFrame*>(NULL))
{
    ...
    else
    {
            ...
            || sensor == System::IMU_RGBD)
        {
            ...
            {
                std::cout
                    << "*Error with the IMU parameters in the config file*"
                    << std::endl;
            }
                                       ...
                                       string         filename)
{
    ...
    else if (mSensor == System::IMU_STEREO && mpCamera2)
    {
        // mCurrentFrame = Frame(mImGray, imGrayRight, timestamp,
        // mpORBextractorLeft, mpORBextractorRight, mpORBVocabulary, mK,
        // mDistCoef, mbf, mThDepth, mpCamera, mpCamera2, mTlr, &mLastFrame,
        // *mpImuCalib);
        mCurrentFrame.reset(mImGray,
                            imGrayRight,
                            timestamp,
                                          ...
                                          string         filename)
{
    ...
    else if (mSensor == System::IMU_MONOCULAR)
    {
        ...
        {
            // mCurrentFrame = Frame(mImGray, timestamp, mpIniORBextractor,
            // mpORBVocabulary, mpCamera, mDistCoef, mbf, mThDepth, &mLastFrame,
            // *mpImuCalib);
            mCurrentFrame.reset(mImGray,
                                timestamp,
                                mpIniORBextractor,
...
void Tracking::PreintegrateIMU()
{
    ...
    auto* pImuPreintegratedFromLastFrame =
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        mpImuPreintegratedFromLastKF->IntegrateNewMeasurement(
            acc, angVel, tstep);
        pImuPreintegratedFromLastFrame->IntegrateNewMeasurement(
            acc, angVel, tstep);
    }
...
bool Tracking::PredictStateIMU()
{
    ...
    else if (!mbMapUpdated)
    {
        ...
        Eigen::Matrix3f Rwb2 = IMU::NormalizeRotation(
            Rwb1
            * imu_preintegrated_frame->GetDeltaRotation(mLastFrame.mImuBias));
        Eigen::Vector3f twb2 =
            twb1 + Vwb1 * t12 + 0.5f * t12 * t12 * Gz
            + Rwb1
                  * imu_preintegrated_frame->GetDeltaPosition(
                      mLastFrame.mImuBias);
        Eigen::Vector3f Vwb2 =
            Vwb1 + t12 * Gz
            + Rwb1
                  * imu_preintegrated_frame->GetDeltaVelocity(
                      mLastFrame.mImuBias);
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if (bOK || mState == RECENTLY_LOST)
        {
            ...
            if (bNeedKF
                && (bOK
                    || (mInsertKFsLost && mState == RECENTLY_LOST
                        && (mSensor == System::IMU_MONOCULAR
                            || mSensor == System::IMU_STEREO
                            || mSensor == System::IMU_RGBD))))
                CreateNewKeyFrame();
...
void Tracking::StereoInitialization()
{
    if (mCurrentFrame.N > 500)
    {
        ...
        Verbose::PrintMess("New Map created with "
                               + to_string(mpAtlas->MapPointsInMap())
                               + " points",
                           Verbose::VERBOSITY_QUIET);
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    float aux = (float)(mCurrentFrame.mTimeStamp - mLastFrame.mTimeStamp)
                / (float)(mCurrentFrame.mTimeStamp - mInitialFrame.mTimeStamp);
    phi *= aux;
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    // std::cout << "NeedNewKF: c1a=" << c1a << "; c1b=" << c1b << "; c1c=" <<
    // c1c << "; c2=" << c2 << std::endl;
...
void Tracking::CreateNewKeyFrame()
{
    ...
    if (mSensor != System::MONOCULAR
        && mSensor
               != System::IMU_MONOCULAR)  // TODO check if incluide imu_stereo
    {
